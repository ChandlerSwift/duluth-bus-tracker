...
.mode csv
select stop_name,stop_id,stop_lat,stop_lon,'$route' as route from stops where stop_id in (
...
echo "name,id,lat,long,route" > stop_data # create header row
...
for stop in stops:
    ...
    stop['led'] = stop_led if stop_led is not "" else 0
...
print(json.dumps(output_routes))
EOF
python3 json_generator.py

echo "Cleaning up..."
rm agency.txt calendar_dates.txt calendar.txt feed_info.txt \
